...
function AddDistict({ isOpen, onClose, children, adddist, handleChangedistrict },props) {
    async function adddistrict(ev) {
        ev.preventDefault();
        ...
        let result = await CallAjax('http://localhost:4050/adddistict',formdata,'POST');
        console.log(result)
        if(result) {
          
          await adddist();

          await handleChangedistrict({value: $("#district").val(), label: $("#district").val()})
        ...
        <form action="" id='Addcity'>
            ...
            <div className="px-10">
                    <label for="name" class="mb-3 block text-base font-medium text-[#07074D]">
                            ...
                            <div class="">
              <input type="text" class="relative bg-gray-50ring-0 outline-none border border-neutral-500 text-neutral-900 placeholder-black-100 text-sm rounded-lg focus:ring-violet-100  focus:border-violet-500 block w-64 p-2.5 checked:bg-emerald-500" placeholder="City..." name='cname' id="district"/>
            </div>
